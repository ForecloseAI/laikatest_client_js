...
class Prompt {
  ...
  async pushScore(scores, session_id = null, user_id = null) {
    // Validate that we have the required metadata
    if (!this._experimentId || !this._bucketId || !this._promptId) {
      throw new Error(
    ...
    // Pass prompt_id (NOT prompt_version_id) to match backend schema
    return await this._client.pushScore(
      this._experimentId,
      this._bucketId,
      this._promptId,  // ✅ Changed from _promptVersionId to _promptId
      scores,
      session_id,
      user_id
    );
