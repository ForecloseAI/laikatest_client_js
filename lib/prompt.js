...
class Prompt {
  ...
  compile(variables) {
    return new Prompt(injectVariables(this._content, variables), this._promptVersionId, this._experimentId, this._bucketId, this._client, this._promptId);
  }
  ...
  async pushScore(scores, options={}) {
    ...
    if (!this._experimentId || !this._bucketId || !this._promptVersionId) {
      throw new Error('Cannot push score: This prompt is not from an experiment. ' + 'Use getExperimentPrompt() to get a scorable prompt.');
    }
    ...
    return await this._client.pushScore(this._experimentId, this._bucketId, this._promptVersionId, scores, options);
  }
}
