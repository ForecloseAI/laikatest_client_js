...
// Validate that at least one of sessionId or userId is provided
function validateSessionOrUserId(sessionId, userId) {
  const hasSessionId = sessionId && typeof sessionId === 'string' && sessionId.trim();
  const hasUserId = userId && typeof userId === 'string' && userId.trim();

  if (!hasSessionId && !hasUserId) {
    throw new ValidationError('At least one of sessionId or userId must be provided');
  }
}
